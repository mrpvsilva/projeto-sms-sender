...
public class JDTelaCadForn extends JDialog implements ActionListener {
	...
	public JDTelaCadForn() throws ParseException {
		...
		JCBTpServ = new JComboBox(f.DDLTipoServico());
	...
	public void actionPerformed(ActionEvent acao) {
		...
		if (acao.getSource() == JBSalvForn) {
			...
			fb.setNomeforn(JTFNome.getText());
			fb.setCnpjforn(JFFCnpj.getText());
			fb.setEmailforn(JTFEmail.getText());
			fb.setEndforn(JTFEnd.getText());
			fb.setTiposervprestadoforn(JCBTpServ.getSelectedItem().toString());
			fb.setTelefonesforn(JFFFone.getText());
			fb.setSiteforn(JTFSite.getText());
			fb.setRgforn(JFFRg.getText());

			String resp = f.Cadastrar(fb);

			if (resp != null)
				JOptionPane.showMessageDialog(null, resp, "Falha",
